...
Feature: Personal auto new submission
    ...
    Scenario: Adding Multiple Vehicles
        Given the user logs into the policy center as "superuser"
        And the user loads "pc" data "newSubmission_06" from json "NewSubmissionTestData"
        When the user creates personal account
    ...
    Scenario: Adding Multiple Drivers
        Given the user logs into the policy center as "superuser"
        And the user loads "pc" data "newSubmission_08" from json "NewSubmissionTestData"
        When the user creates personal account
